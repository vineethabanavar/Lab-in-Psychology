...
expName = 'Canli_replication'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/vineesmacbook/Desktop/PsychoPy/Solo Project/PsychoPy experiment files/Phase1_lastrun.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('rating_resp') is None:
        # initialise rating_resp
        rating_resp = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='rating_resp',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    instructions = visual.TextStim(win=win, name='instructions',
        text='Welcome to the experiment.\n\nYou will see a series of photographs, one at a time. Your task is to rate how emotionally intense each picture feels to you.\n\nUse the number keys from 1 to 4: \n1 = not emotionally intense at all\n4 = extremely emotionally intense\n\nThere are no right or wrong answers — please respond based on your immediate feeling. \n\n\nPress SPACE to begin.',
        font='Arial',
    ...
    # --- Initialize components for Routine "image_rating" ---
    # Run 'Begin Experiment' code from loadImages
    from random import shuffle
    from pathlib import Path
    
    # Point to your 'photos' folder
    image_folder = Path('photos')
    
    # Get all image files (jpg/png/jpeg)
    image_list = [f for f in image_folder.glob('*.jpg')] + \
                 [f for f in image_folder.glob('*.png')] + \
                 [f for f in image_folder.glob('*.jpeg')]
    
    # Randomize their order
    shuffle(image_list)
    
    # Convert to strings so PsychoPy can use them
    ...
    rating_resp = keyboard.Keyboard(deviceName='rating_resp')
    ...
    for thisTrial in trials:
        ...
        # --- Prepare to start Routine "image_rating" ---
        # create an object to store info about Routine image_rating
        image_rating = data.Routine(
            name='image_rating',
            components=[image, fixation, rating_resp],
        )
        image_rating.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from loadImages
        # Reset variables every trial
        event.clearEvents()  # ensures no leftover keys from previous image
        rating = None
        rating_time = None
        ...
        # create starting attributes for rating_resp
        rating_resp.keys = []
        rating_resp.rt = []
        _rating_resp_allKeys = []
        # store start times for image_rating
        image_rating.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        image_rating.tStart = globalClock.getTime(format='float')
        image_rating.status = STARTED
        thisExp.addData('image_rating.started', image_rating.tStart)
        image_rating.maxDuration = None
        # keep track of which components have finished
        image_ratingComponents = image_rating.components
        for thisComponent in image_rating.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "image_rating" ---
        image_rating.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 7.0:
            ...
            frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
            # update/draw components on each frame
            # Run 'Each Frame' code from loadImages
            # Only allow responses after the image has disappeared
            if image.status == FINISHED:
                if rating is None:
                    keys = event.getKeys(keyList=['1', '2', '3', '4'])
                    if keys:
                        rating = keys[-1]
                        rating_time = clock.getTime()
                        fixation.color = 'green'
            
            # Keep feedback visible for 0.5 s, then revert
            if rating_time is not None:
                if clock.getTime() - rating_time > 0.5:
                    fixation.color = 'white'
                    
            # Ignore all keys during image display
            ...
            if fixation.status == NOT_STARTED and tThisFlip >= 3.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            # if rating_resp is starting this frame...
            if rating_resp.status == NOT_STARTED and tThisFlip >= 3-frameTolerance:
                # keep track of start time/frame for later
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'rating_resp.started')
                # update status
            ...
            # if rating_resp is stopping this frame...
            if rating_resp.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rating_resp.tStartRefresh + 4-frameTolerance:
                    # keep track of stop time/frame for later
                    rating_resp.tStop = t  # not accounting for scr refresh
                    rating_resp.tStopRefresh = tThisFlipGlobal  # on global time
                    rating_resp.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'rating_resp.stopped')
                    # update status
                    rating_resp.status = FINISHED
                    rating_resp.status = FINISHED
            if rating_resp.status == STARTED and not waitOnFlip:
                theseKeys = rating_resp.getKeys(keyList=['1','2','3','4'], ignoreKeys=["escape"], waitRelease=False)
                ...
                if len(_rating_resp_allKeys):
                    ...
                    rating_resp.duration = _rating_resp_allKeys[-1].duration
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                image_rating.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in image_rating.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        # --- Ending Routine "image_rating" ---
        for thisComponent in image_rating.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for image_rating
        image_rating.tStop = globalClock.getTime(format='float')
        image_rating.tStopRefresh = tThisFlipGlobal
        thisExp.addData('image_rating.stopped', image_rating.tStop)
        # Run 'End Routine' code from loadImages
        from pathlib import Path
        
        # Save key data
        thisExp.addData('trial_num', trials.thisN + 1)
        thisExp.addData('image_name', Path(imageFile).name)
        thisExp.addData('rating', rating if rating is not None else 'NA')
        
        # Optional response info
        thisExp.addData('response_given', 1 if rating else 0)
        # check responses
        ...
        if rating_resp.keys != None:  # we had a response
            ...
            trials.addData('rating_resp.duration', rating_resp.duration)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if image_rating.maxDurationReached:
            routineTimer.addTime(-image_rating.maxDuration)
        elif image_rating.forceEnded:
            routineTimer.reset()
        else:
